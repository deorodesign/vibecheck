...
// Výpočet hodností podle objemu sázek
    ...
    <div className={`flex gap-3 ${isReply ? 'ml-10 mt-2 border-l-2 border-zinc-800 pl-4' : 'mt-6'}`}>
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          <Link href={`/user/${encodeURIComponent(msg.user)}`} className="font-bold text-sm hover:underline text-white uppercase italic">
            {msg.user}
          </Link>
        ...
        <div className="flex items-center gap-4 opacity-60 hover:opacity-100 transition-opacity">
          ...
          <button 
            ...
            className={`flex items-center gap-1 text-[10px] font-bold uppercase transition-colors ${isLiked ? 'text-fuchsia-500' : 'text-zinc-500 hover:text-white'}`}
          >
...
export default function Chat({ marketId }: { marketId: number }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#18181b] rounded-[2rem] border border-white/5 overflow-hidden shadow-xl">
      {/* KOLONKA NAHOŘE */}
      <div className="p-4 bg-black/20 border-b border-white/5">
        ...
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-fuchsia-500" 
          />
          <button onClick={handleSend} className="px-5 py-3 bg-white text-black rounded-xl text-xs font-black uppercase tracking-widest active:scale-95 transition-all">Send</button>
        </div>
      </div>
