    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/80 dark:bg-black/80 backdrop-blur-sm" onClick={() => setIsLoginModalOpen(false)}>
      <div className="bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/10 rounded-[2rem] p-8 max-w-sm w-full shadow-2xl flex flex-col gap-4 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <button onClick={loginWithGoogle} className="flex items-center justify-center gap-4 w-full py-4 rounded-xl bg-white text-black border border-zinc-200 font-black uppercase tracking-widest text-xs shadow-md active:scale-95 transition-all">
            ...
            <div className="flex flex-col gap-5 -mt-16 md:-mt-20 relative z-10 px-0 md:px-8">
              ...
              <div className="bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/5 rounded-[2rem] p-5 md:p-6 shadow-md mx-4 md:mx-0">
                ...
                <div className="relative h-12 bg-zinc-100 dark:bg-black/50 rounded-2xl overflow-hidden flex items-center mb-6 border border-zinc-200 dark:border-white/5 shadow-inner">
                  <div className="h-full bg-green-500 flex items-center px-4 transition-all duration-500 ease-out shadow-[0_0_20px_rgba(34,197,94,0.6)]" style={{ width: `${(currentPrices?.vibe || 0.5) * 100}%` }}><span className="text-white dark:text-black font-black italic text-sm z-10">{((currentPrices?.vibe || 0.5) * 100).toFixed(0)}%</span></div>
                                ...
                                <div className="flex flex-col gap-3 ml-8 pl-3 border-l border-zinc-100 dark:border-white/5 mt-1">
                                  {replies.map((reply: any) => (
                                    <div key={reply.id} className="flex items-start gap-2">
                                      ...
                                      <div className="flex flex-col gap-0.5 w-full"><div className="flex items-center gap-2"><span className="font-black uppercase text-[8px] text-zinc-400">{reply.user}</span><span className="text-[7px] text-zinc-500 font-medium">{formatTimeAgo(reply.timestamp)}</span></div><span className="text-zinc-600 dark:text-zinc-300 font-medium bg-zinc-50 dark:bg-white-[0.02] p-2 rounded-xl rounded-tl-sm border border-zinc-100 dark:border-white/5 text-[10px]">{reply.text}</span></div>
                                    </div>
                                  ))}
                                </div>
                              )}
