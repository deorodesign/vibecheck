...
import Chat from './components/chat'; // Import tvého vylepšeného chatu
...
function HomeContent() {
  ...
  const {
    ...
    marketPrices, myBets, placeBet, selectedMarket, setSelectedMarket, 
    avatarUrl, nickname, isDarkMode, toggleDarkMode, marketStatus, 
    dynamicLeaderboard, showToast, isLoginModalOpen, setIsLoginModalOpen,
  ...
  // HEADER (Tlačítko profilu a Bankroll)
  const headerContent = (
      ...
      <div className="w-full max-w-7xl flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {isLoggedIn ? (
            ...
            <button onClick={() => setIsLoginModalOpen(true)} className="px-6 h-10 rounded-full bg-zinc-900 text-white dark:bg-white dark:text-black text-xs font-black uppercase tracking-widest hover:scale-105 transition-all">Log In</button>
          )}
        </div>
      </div>
      {!selectedMarket && (
        <div className="w-full max-w-7xl overflow-x-auto flex gap-2 pb-2 no-scrollbar">
          {CATEGORIES.map(cat => (
  ...
  const loginModalContent = isLoginModalOpen && (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsLoginModalOpen(false)}>
      <div className="bg-white dark:bg-[#18181b] border border-white/10 rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl flex flex-col gap-4 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-3">
          <button onClick={loginWithTwitter} className="w-full py-4 rounded-2xl bg-black dark:bg-white text-white dark:text-black font-black uppercase tracking-widest text-xs shadow-md active:scale-95">Continue with X</button>
          <button onClick={loginWithGoogle} className="flex items-center justify-center gap-4 w-full py-4 rounded-2xl bg-white text-black border border-zinc-200 font-black uppercase tracking-widest text-xs shadow-md active:scale-95">
          ...
          <button onClick={loginWithDiscord} className="w-full py-4 rounded-2xl bg-[#5865F2] text-white font-black uppercase tracking-widest text-xs shadow-md">Continue with Discord</button>
        </div>
        <div className="relative flex items-center py-4 opacity-20"><div className="flex-grow border-t dark:border-white"></div><span className="mx-4 text-[9px] font-black uppercase">OR</span><div className="flex-grow border-t dark:border-white"></div></div>
        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row items-start gap-8 py-6 px-4">
          <div className="w-full lg:flex-1 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-5 px-0 md:px-8 -mt-16 md:-mt-24 relative z-10">
              <h1 className="text-3xl md:text-5xl font-black uppercase italic drop-shadow-2xl text-zinc-900 dark:text-white">{selectedMarket.title}</h1>
              <div className="bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/5 rounded-[2.5rem] p-8 shadow-xl mx-4 md:mx-0">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={(e) => handleVote(e, selectedMarket.id, 'VYBE')} className="p-6 rounded-[1.5rem] bg-green-500 text-white font-black text-2xl uppercase italic shadow-lg">VYBE</button>
                  <button onClick={(e) => handleVote(e, selectedMarket.id, 'NO_VYBE')} className="p-6 rounded-[1.5rem] bg-red-500 text-white font-black text-2xl uppercase italic shadow-lg">NO VYBE</button>
                </div>
              </div>
            </div>
          </div>
          {/* VOLÁNÍ EXTERNÍHO CHATU */}
          ...
          {markets.filter(m => activeCategory === 'All' || m.category === activeCategory).map((market: any) => (
            <div key={market.id} onClick={() => openMarket(market)} className="group bg-white dark:bg-[#18181b] rounded-[2.5rem] overflow-hidden border border-zinc-200 dark:border-white/5 cursor-pointer hover:shadow-2xl transition-all">
              <div className="aspect-video relative overflow-hidden bg-zinc-200 dark:bg-zinc-900">
                ...
                <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full text-[9px] font-black text-white border border-white/10 uppercase italic tracking-widest">Vol: ${market.volumeUsd || market.volume_usd || 0}</div>
              </div>
              <div className="p-8">
