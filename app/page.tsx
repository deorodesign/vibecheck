...
function HomeContent() {
  ...
  const {
    markets, isLoggedIn, isAuthLoading, balance, handleLogout, marketPrices, myBets, placeBet, 
    selectedMarket, setSelectedMarket, nickname, avatarUrl, marketStatus, dynamicLeaderboard, 
    showToast, isLoginModalOpen, setIsLoginModalOpen, loginWithTwitter, loginWithDiscord, loginWithEmail, loginWithGoogle
  } = useAppContext();
          ...
          {dynamicLeaderboard.map((user: any) => (
            <div key={user.id} className="flex items-center justify-between p-4 rounded-2xl hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-4">
                <span className={`font-black italic text-lg w-4 ${user.rank === 1 ? 'text-yellow-500' : 'text-zinc-300'}`}>{user.rank}</span>
              ...
              <span className="font-black font-mono text-sm">{user.points.toLocaleString('en-US')}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <main className="flex min-h-screen flex-col items-center bg-zinc-50 dark:bg-[#0e0e12] transition-colors duration-500">
      {!selectedMarket && (
        <div className="w-full max-w-7xl flex overflow-x-auto gap-2 px-4 md:px-8 py-4 no-scrollbar">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`whitespace-nowrap px-5 py-2.5 rounded-full text-xs font-bold transition-all shadow-sm ${activeCategory === cat ? 'bg-zinc-900 text-white dark:bg-white dark:text-black shadow-lg' : 'bg-white dark:bg-white/5 text-zinc-500 border border-zinc-200 dark:border-white/10'}`}>{cat}</button>
          ))}
        </div>
      )}
          ...
          <div className="w-full lg:flex-1 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-5 px-0 md:px-8 -mt-16 md:-mt-24 relative z-10">
              ...
              <div className="bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/5 rounded-[2.5rem] p-8 shadow-xl mx-4 md:mx-0">
                <div className="relative h-14 bg-zinc-100 dark:bg-black/50 rounded-2xl overflow-hidden flex items-center mb-8">
                ...
                <div className="flex gap-3 mb-6">
                  <input type="number" value={betAmount} onChange={(e) => setBetAmount(e.target.value)} className="flex-1 bg-zinc-50 dark:bg-black border border-zinc-200 dark:border-white/10 rounded-2xl px-6 py-4 font-mono font-black text-lg text-zinc-900 dark:text-white" />
                </div>
            ...
            {markets.filter((m: any) => activeCategory === 'All' || m.category === activeCategory).map((market: any) => (
              <div key={market.id} onClick={() => openMarket(market)} className="group bg-white dark:bg-[#18181b] rounded-[2.5rem] overflow-hidden border border-zinc-200 dark:border-white/5 cursor-pointer hover:shadow-2xl transition-all">
                <div className="aspect-video relative overflow-hidden bg-zinc-200 dark:bg-zinc-900">
                  ...
                  <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full text-[9px] font-black text-white border border-white/10 uppercase italic">Vol: ${market.volumeUsd || market.volume_usd || 0}</div>
                </div>
                <div className="p-8">
...
export default function Home() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0e0e12] flex items-center justify-center font-black uppercase italic text-fuchsia-500">Loading Vybecheck...</div>}>
      <HomeContent />
    </Suspense>
  );
}