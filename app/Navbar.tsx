          ...
          {isAuthLoading ? (
            ...
            <>
              <div className="flex items-center gap-3 bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 px-4 md:px-5 py-2.5 rounded-full shadow-sm">
                <span className="text-xs md:text-sm font-mono font-bold text-zinc-900 dark:text-white">{(balance || 0).toFixed(2)} USDC</span>
              </div>
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-10 h-10 rounded-full border border-zinc-200 dark:border-white/10 overflow-hidden shadow-sm">
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
                    ...
                    <Link href="/profile" onClick={() => setIsProfileOpen(false)} className="block px-4 py-3 text-xs font-bold hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors">Profile Settings</Link>
                    <button onClick={() => { handleLogout(); setIsProfileOpen(false); }} className="w-full text-left px-4 py-3 text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors">Log Out</button>
                  </div>
                )}
              </div>
            </>
